...
1. 本地已完成"文件清单报价缓存"（未提交部署）：投标文件解析完成时提取一次报价并缓存到文件清单（万元显示，悬停可见精确元），价格工作表复用同一缓存、只在文件/解析指纹变化或强制"重新识别"时重扫并回写，报告文件清单同步显示报价；旧文件由价格表刷新补提取。新增 4 项测试，全量 564 项通过，待提交部署。
2. 云端已部署 `7d18ac7`（提示词 v63 未填调整不套用规则优惠）：build-info `version_consistent=true`、`prompt_version=vision-evidence-contract-v63`。下一步建议：重跑 sxyh2 核对（期望 4×30），并在 test3 人工复填调整后复跑核对提示与分数。
2. 云端已部署 `76c31e0`（空专用集回退完整集）：test3 恢复正确（基准价 1652677.4737、43.63/44.13/36.14/37.01）；jc 回显修复已生效；sxyh2 复跑应为 4×30（待重跑核对）。

## 活跃记录（最多 10 条）

### 5. 文件清单报价缓存（本地已完成，待提交部署）
...
- 文件清单新增"报价（万元）"列（悬停显示精确元与来源），报告文件清单同步显示；旧文件（解析时未缓存）由价格表刷新补提取。通用机制，不针对具体项目。
