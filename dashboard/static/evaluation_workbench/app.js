(() => {
  ...
  function resetProjectPanels() { ['documents','rules','review-results','objective-results','subjective-results'].forEach((id) => { const node = $(id); if (node) node.innerHTML = '<p class="muted">正在加载当前项目…</p>'; }); $('token-usage').textContent = '正在加载当前项目…'; $('latest-run-usage').textContent = '正在加载当前项目…'; $('task-status').textContent = '正在加载当前项目…'; lastPartialDocumentsKey = ''; }
  ...
  async function renderCompare(taskId, documents) {
    ...
    const signalRows = (items) => items.map((signal) => { const assessment = signal.ai_assessment || {}; return `<tr><td>${escapeHtml(signal.bidder_a)}${signal.bidder_b ? ` ↔ ${escapeHtml(signal.bidder_b)}` : ''}<br><span class="tag">${escapeHtml(signal.dimension_label)}</span></td><td>${escapeHtml(signal.basis)}<pre class="evidence">${escapeHtml(evidenceText(signal) || '详见原始文件对结果')}</pre></td><td>${escapeHtml(aiDecision[assessment.decision] || '等待 AI 判定')}<br><small>${escapeHtml(assessment.reason || '')}</small><br><small>来源：${escapeHtml(compareSourceLabel(assessment.source_class))} · ${escapeHtml(compareMaterialityLabel(assessment.materiality))}<br>风险：${escapeHtml(riskLabel(assessment.risk_level))} · 置信度：${escapeHtml(confidenceLabel(assessment.confidence))}<br>${escapeHtml(assessment.suggested_check || '')}</small></td><td>${escapeHtml((signal.counter_evidence || []).join('；') || '-')}</td></tr>`; }).join('');
    const primarySignals = orderedSignals.filter((signal) => signal.display_group !== 'low_value');
    const lowValueSignals = orderedSignals.filter((signal) => signal.display_group === 'low_value');
    const signalTable = signals.length ? `<h4>固定规则线索与 AI 判定</h4><p class="hint">AI仅复核本地算法提取的短证据包，不读取完整文件对；本次为纯文字查重，不调用 OCR 或图片识别。${ai.status === 'success' ? `已判定 ${ai.assessed_count || 0} 条线索。` : escapeHtml(ai.reason || '')}</p>${primarySignals.length ? `<table><thead><tr><th>文件对 / 维度</th><th>证据</th><th>AI 判定</th><th>反证提示</th></tr></thead><tbody>${signalRows(primarySignals)}</tbody></table>` : '<p class="muted">没有需要优先复核的有效线索。</p>'}${lowValueSignals.length ? `<details><summary>公共来源、共同第三方资料或占位/提取伪影（${lowValueSignals.length} 条，不计入复核优先级）</summary><table><thead><tr><th>文件对 / 维度</th><th>证据</th><th>AI 判定</th><th>反证提示</th></tr></thead><tbody>${signalRows(lowValueSignals)}</tbody></table></details>` : ''}` : '<p class="muted">本次未检出可报告的横向异常线索。未检出不等同于不存在其他风险。</p>';
