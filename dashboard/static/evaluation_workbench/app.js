(() => {
  ...
  const acquisitionPresetLabels = {smart:'智能升级', always:'每次都升级', text:'腾讯 OCR 字段复核', visual:'签章/外观核验', dual:'双重增强核验', off:'不作增强核验', custom:'自定义高级设置'};
  ...
  async function refreshRules() {
    ...
    const toolbar = isDraft && displayRules.length ? `<div class="rule-acquisition-toolbar"><div><strong>增强核验设置</strong><span class="muted"> 本地 OCR 仅在明确需要或文字证据不足时运行；日常只选择是否升级以及强度，腾讯 OCR、多模态通道由系统判断。</span><small>当前：不作增强 ${acquisitionCounts.off || 0} 条 · 智能升级 ${acquisitionCounts.smart || 0} 条 · 每次都升级 ${acquisitionCounts.always || 0} 条${acquisitionCounts.custom ? ` · 专家自定义 ${acquisitionCounts.custom} 条` : ''}</small><small>${escapeHtml(acquisitionCapabilityNote())}</small></div><div class="rule-acquisition-toolbar-actions"><button class="restore-acquisition-recommendations" type="button">已启用规则采用系统建议</button><details class="rule-acquisition-help"><summary>如何选择？</summary><div class="rule-acquisition-help-popover"><p><strong>不作增强核验：</strong>先审全文，只有明确需 OCR、扫描型文件或文字证据不足时才调用本地 OCR。<strong>智能升级：</strong>适合大多数规则，关键字段、识别不完整或外观事实需要时才追加复核。<strong>每次都升级：</strong>每次评审均追加复核，适合签章、勾选或图片材料不能漏看的事项。<strong>快速／标准／充分：</strong>分别适合单页抽查、常规材料、材料分散或风险较高的情形。</p><p>关闭腾讯 OCR 或多模态不会影响必要的本地基础识别；不能执行的增强通道会在规则卡片中明确提示。</p></div></details></div></div>` : '';
