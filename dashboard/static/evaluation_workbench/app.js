(() => {
  ...
  const acquisitionPresetLabels = {smart:'智能取证', always:'每次都取证', text:'扫描文字核验', visual:'签章/外观核验', dual:'双重核验', off:'不进行图片取证', custom:'自定义高级设置'};
  ...
  function acquisitionSummary(rule) {
    ...
    return {label:`${acquisitionPresetLabels[choice]} · ${acquisitionLevelLabels[level] || '标准'}强度`, state:'active', detail:choice === 'always' ? '每次评审均检查候选材料，由系统选择必要通道' : '证据不足时按需执行'};
  }
  ...
  function acquisitionPreview(rule) {
    ...
    return `${acquisitionRecommendationScene(rule)} ${path}；${budget}。`;
  }
  ...
  async function refreshRules() {
    ...
    const toolbar = isDraft && displayRules.length ? `<div class="rule-acquisition-toolbar"><div><strong>图片取证设置</strong><span class="muted"> 日常仅选“智能取证／每次都取证”和强度，OCR、多模态通道由系统判断。</span><small>当前：不启用 ${acquisitionCounts.off || 0} 条 · 智能 ${acquisitionCounts.smart || 0} 条 · 每次都取证 ${acquisitionCounts.always || 0} 条${acquisitionCounts.custom ? ` · 专家自定义 ${acquisitionCounts.custom} 条` : ''}</small><small>${escapeHtml(acquisitionCapabilityNote())}</small></div><div class="rule-acquisition-toolbar-actions"><button class="restore-acquisition-recommendations" type="button">已启用规则采用系统建议</button><details class="rule-acquisition-help"><summary>如何选择？</summary><div class="rule-acquisition-help-popover"><p><strong>智能取证：</strong>适合大多数规则，证据不足时才处理候选图片。<strong>每次都取证：</strong>每次评审均检查候选材料，适合签章、勾选、图片材料等不能漏看的事项。<strong>快速／标准／充分：</strong>分别适合单页抽查、常规材料、材料分散或风险较高的情形。</p><p>全局未开启 OCR 或多模态时，能执行的通道仍会继续；不能执行的纯视觉核验会在规则卡片中明确提示。</p></div></details></div></div>` : '';
    $('rules').innerHTML = displayRules.length ? `${toolbar}<div class="rule-card-list">${displayRules.map((r) => {
      ...
      const choiceControl = isCustomAcquisition ? `<div class="rule-acquisition-custom-state"><strong>专家自定义生效中</strong><button class="open-rule-expert" data-rule="${r.rule_id}" type="button">查看/调整专家模式</button></div>` : `<label>图片取证<select class="rule-simple-acquisition" data-rule="${r.rule_id}"><option value="off" ${simpleChoice === 'off' ? 'selected' : ''}>不启用</option><option value="smart" ${simpleChoice === 'smart' ? 'selected' : ''}>智能取证（推荐）</option><option value="always" ${simpleChoice === 'always' ? 'selected' : ''}>每次都取证</option></select></label>`;
