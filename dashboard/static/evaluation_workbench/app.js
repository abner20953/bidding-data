(() => {
  ...
  function renderOcrConfiguration() {
    const tencentEnabled = Boolean(ocrConfiguration.tencent_enabled ?? ocrConfiguration.enabled); $('ocr-enabled').checked = tencentEnabled; $('ocr-region').value = ocrConfiguration.region || 'ap-guangzhou';
  ...
  async function refreshRules() {
    ...
    $('rules').innerHTML = displayRules.length ? `<div class="rule-card-list">${displayRules.map((r) => {
      ...
      const visionControl = isDraft ? `<div class="rule-vision-controls"><label>图片取证方式<select class="rule-image-mode" data-rule="${r.rule_id}"><option value="auto" ${r.image_mode === 'auto' ? 'selected' : ''}>自动协同</option><option value="ocr_only" ${r.image_mode === 'ocr_only' ? 'selected' : ''}>仅 OCR 文字识别</option><option value="vision_only" ${r.image_mode === 'vision_only' ? 'selected' : ''}>仅多模态图片识别</option><option value="combined" ${r.image_mode === 'combined' ? 'selected' : ''}>OCR＋多模态均核验</option><option value="off" ${r.image_mode === 'off' ? 'selected' : ''}>不使用图片</option></select></label><label>图片识别条件<select class="rule-vision-trigger" data-rule="${r.rule_id}"><option value="off" ${r.vision_trigger === 'off' ? 'selected' : ''}>不使用图片</option><option value="text_fallback" ${r.vision_trigger === 'text_fallback' ? 'selected' : ''}>文字不足时识图</option><option value="required" ${r.vision_trigger === 'required' ? 'selected' : ''}>必须识图</option></select></label><label>识图强度<select class="rule-vision-level" data-rule="${r.rule_id}" ${r.vision_trigger === 'off' || r.image_mode === 'off' ? 'disabled' : ''}><option value="off" ${r.vision_level === 'off' ? 'selected' : ''}>暂不执行</option><option value="low" ${r.vision_level === 'low' ? 'selected' : ''}>快速（最多2页，不补页）</option><option value="standard" ${r.vision_level === 'standard' ? 'selected' : ''}>标准（首批最多4页，可补4页）</option><option value="high" ${r.vision_level === 'high' ? 'selected' : ''}>精细（首批最多6页，可补6页并支持扫描件找页）</option></select></label><small>OCR：${ocrFeatureConfiguration.enabled ? '已开启（腾讯云优先、本地 RapidOCR 回退）' : '已关闭'}；多模态：${visionConfiguration.enabled ? '已开启，可核验签章、版式等外观事实' : '已关闭，纯 OCR 不受影响'}。仅 OCR 不会发送图片给模型。</small></div>` : '';
    ...
    const ruleImagePayload = (ruleId, extra = {}) => { const mode = $('rules').querySelector(`.rule-image-mode[data-rule="${ruleId}"]`)?.value || 'auto'; const trigger = $('rules').querySelector(`.rule-vision-trigger[data-rule="${ruleId}"]`)?.value || 'off'; const level = $('rules').querySelector(`.rule-vision-level[data-rule="${ruleId}"]`)?.value || 'off'; return {...extra, image_mode:mode, vision_trigger:mode === 'off' ? 'off' : trigger, vision_level:(mode === 'off' || trigger === 'off') ? 'off' : level}; };
  ...
  $('save-ocr-feature-configuration').onclick = async () => { try { await request('/ocr-feature-configuration', {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({enabled:$('ocr-feature-enabled').checked})}); await loadProfiles(); } catch (error) { alert(error.message); } };
  ...
  $('save-ocr-configuration').onclick = async () => { try { const services = {}; $('ocr-services').querySelectorAll('.ocr-service-enabled').forEach((input) => { const limit = $('ocr-services').querySelector(`.ocr-service-limit[data-service="${input.dataset.service}"]`); services[input.dataset.service] = {enabled:input.checked, monthly_limit:Number(limit?.value || 900)}; }); await request('/tencent-ocr-configuration', {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({tencent_enabled:$('ocr-enabled').checked, region:$('ocr-region').value, secret_id:$('ocr-secret-id').value, secret_key:$('ocr-secret-key').value, services})}); $('ocr-secret-id').value = ''; $('ocr-secret-key').value = ''; await loadProfiles(); } catch (error) { alert(error.message); } };
