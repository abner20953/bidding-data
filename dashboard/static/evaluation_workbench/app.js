(() => {
  ...
  function taskText(task, queueContext = null) {
    if (!task) return '暂无任务';
    const labels = {queued:'排队中',running:'运行中',success:'已完成',error:'失败',cancelled:'已取消',interrupted:'已中断'};
    const partial = task?.result?.completion_state === 'partial_success';
    const cancelling = task.task_type === 'evaluate_all' && task.status === 'running' && task.cancel_requested;
    const failed = partial && Array.isArray(task?.result?.failed_units) ? task.result.failed_units.length : 0;
    const label = cancelling ? '正在安全终止' : (partial ? '部分完成' : (labels[task.status] || escapeHtml(task.status)));
    const retry = partial && task.task_type === 'evaluate_all' ? `<br><button class="retry-failed-evaluation" data-task="${escapeHtml(task.task_id)}">仅重跑失败项</button>` : '';
    const canCancel = task.task_type === 'evaluate_all' && ['queued','running'].includes(task.status) && !task.cancel_requested;
    const cancel = canCancel ? `<button class="cancel-evaluation-task danger" data-task="${escapeHtml(task.task_id)}" data-status="${escapeHtml(task.status)}">${task.status === 'queued' ? '取消排队' : '安全终止'}</button>` : '';
    const taskMessage = cancelling ? '已收到终止请求，正在等待当前调用结束' : (task.message || '');
    return `<span class="status-${task.status}">${label} ${task.progress || 0}% ${escapeHtml(taskMessage)}${cancel}${cancelling ? '<br><small>不会再启动新的模型或 OCR 调用；当前调用返回后停止。</small>' : ''}${partial ? `<br><small>有 ${failed} 个规则组可单独重跑；其余结果已保留。</small>` : ''}${retry}${task.status === 'queued' ? queueDetail(queueContext) : ''}${taskElapsed(task)}${task.error ? `<br>${escapeHtml(task.error)}` : ''}</span>`;
  }
  ...
  function renderPriceSheetPane() {
    ...
    const ruleMentionsAdjustment = rule ? /(?:中小企业|价格扣除|优惠扣除|剔除税率|换算为不含税|税率不同|价格优惠)/.test(`${rule.title || ''} ${rule.check_rule || ''} ${rule.source_text || ''}`) : false;
    const unadjustedIncluded = entries.filter((entry) => entry.included && (!entry.adjustment || !entry.adjustment.mode || entry.adjustment.mode === 'none')).length;
    const adjustmentHint = (ruleMentionsAdjustment && unadjustedIncluded > 0) ? `<p class="price-sheet-warning">规则文本提到价格优惠/税率换算（如中小企业扣除、不含税价计算），但仍有 ${unadjustedIncluded} 家参与计算的投标人未填写“计分价调整”。未填写的将按原计分价计算、不会自动套用规则里的优惠或税率；请确认是否需要补填。</p>` : '';
    target.innerHTML = `<div class="price-sheet-toolbar">${rulePicker}${ruleDetails}</div>${summary}${coverageWarning}${adjustmentHint}<p class="price-sheet-note">${escapeHtml(currentPriceSheet.notice || '')} 价格优惠、税率差异等由人工选择并填写依据；系统只按明确输入统一计算，不自动判断政策资格。</p><div class="price-table-wrap"><table class="price-table price-workbench-table"><thead><tr><th>投标人</th><th>确认报价</th><th>计分价调整</th><th>参与范围</th><th>价格分</th></tr></thead><tbody>${rows}</tbody></table></div>`;
  ...
  function partialResultNotice(run) {
    ...
    if (run?.task_status === 'cancelled') return `${baseNotice}<p class="hint">本次综合评审已安全终止；已完整完成的投标人结果已保留，未完成部分仍显示上次成功结果或标记为尚未评审。</p>`;
