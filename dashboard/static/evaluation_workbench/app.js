(() => {
  ...
  const acquisitionPresetLabels = {recommended:'按 AI 建议', adaptive:'文字优先', text_only:'纯文字核验', local_ocr:'本地 OCR 核验', smart:'智能升级', always:'每次都升级', text:'腾讯 OCR 字段复核', visual:'签章/外观核验', dual:'双重增强核验', off:'不作增强核验', custom:'自定义高级设置'};
  ...
  async function refreshRules() {
    ...
    const toolbar = isDraft && displayRules.length ? `<div class="rule-acquisition-toolbar"><div><strong>核验方式</strong><span class="muted"> 默认按 AI 对证据类型的判断执行；也可明确限定为纯文字、本地 OCR，或追加腾讯 OCR／多模态增强。</span><small>当前：AI 建议 ${acquisitionCounts.recommended || 0} 条 · 文字优先 ${acquisitionCounts.adaptive || 0} 条 · 纯文字 ${acquisitionCounts.text_only || 0} 条 · 本地 OCR ${acquisitionCounts.local_ocr || 0} 条 · 智能增强 ${acquisitionCounts.smart || 0} 条 · 每次增强 ${acquisitionCounts.always || 0} 条${acquisitionCounts.custom ? ` · 专家自定义 ${acquisitionCounts.custom} 条` : ''}</small><small>${escapeHtml(acquisitionCapabilityNote())}</small></div><div class="rule-acquisition-toolbar-actions"><button class="restore-acquisition-recommendations" type="button">已启用规则采用 AI 建议</button><details class="rule-acquisition-help"><summary>如何选择？</summary><div class="rule-acquisition-help-popover"><p><strong>按 AI 建议：</strong>系统依据规则自动选择。<strong>文字优先：</strong>正常只审全文，文字覆盖不足才补本地 OCR。<strong>纯文字核验：</strong>只审已解析全文。<strong>本地 OCR 核验：</strong>固定追加有限候选页本地识别。<strong>智能增强／每次都增强：</strong>按需或固定追加腾讯 OCR／多模态。</p><p>快速／标准／充分只控制增强核验的页数上限；本地 OCR 仍按有限候选页和缓存执行。</p></div></details></div></div>` : '';
