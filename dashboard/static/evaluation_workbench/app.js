(() => {
  ...
  function resetProjectPanels() { ['documents','rules','review-results','objective-results','subjective-results'].forEach((id) => { const node = $(id); if (node) node.innerHTML = '<p class="muted">正在加载当前项目…</p>'; }); $('token-usage').textContent = '模型用量：正在加载当前项目…'; $('latest-run-usage').textContent = '最近一次模型用量：正在加载当前项目…'; $('task-status').textContent = '正在加载当前项目…'; lastPartialDocumentsKey = ''; }
