(() => {
  ...
  async function refreshUsage() { if (!activeProject) return; const data = await request(`/projects/${activeProject}/token-usage`); const u = data.usage; if (!u.call_count && !u.ocr_requests) { $('token-usage').textContent = '模型用量：尚无调用记录'; return; } const detail = u.metered_calls ? `输入 ${u.prompt_tokens.toLocaleString()} / 输出 ${u.completion_tokens.toLocaleString()} / 合计 ${u.total_tokens.toLocaleString()} Token` : `模型接口未返回 Token；已发送 ${u.input_chars.toLocaleString()} 字符`; const families = u.families || {}; const extras = []; if (families.vision && families.vision.call_count) extras.push(`图片识别 ${families.vision.call_count} 次`); if (u.ocr_requests) extras.push(`腾讯 OCR ${u.ocr_requests} 页`); $('token-usage').textContent = `模型用量：${detail}（${u.call_count} 次调用${extras.length ? '；其中' + extras.join('、') : ''}）`; }
