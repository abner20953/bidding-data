(() => {
  ...
  let ocrFeatureConfiguration = {enabled:false};
  ...
  function renderOcrConfiguration() {
    ...
    $('ocr-feature-enabled').checked = Boolean(ocrFeatureConfiguration.enabled ?? ocrConfiguration.ocr_enabled);
    $('ocr-route-tencent').checked = tencentEnabled;
    $('ocr-route-local').checked = !tencentEnabled;
    $('ocr-route-controls').disabled = !ocrFeatureConfiguration.enabled;
    const source = {manual:'手动保存', environment:'运行环境变量', none:'未配置'}[ocrConfiguration.credentials_source] || '未配置';
    const imageGate = ocrFeatureConfiguration.enabled ? 'OCR 文字识别总开关已开启。' : '提示：OCR 文字识别总开关当前关闭，评审中不会调用 OCR。';
    ...
    $('local-ocr-status').textContent = localReady ? `本地 RapidOCR 已就绪：${tencentEnabled ? '作为腾讯云 OCR 的自动回退' : '当前作为直接 OCR 路径'}，按批启动、结束即释放内存。${readinessHint}` : '本地 RapidOCR 运行环境未就绪；重新部署后会自动恢复。腾讯云关闭时将无法执行 OCR。';
    $('ocr-configuration-hint').textContent = `腾讯云状态：${tencentEnabled ? `已启用（凭据：${source}）` : '未启用，本次不会发送腾讯云 OCR 请求'}。本月 ${ocrConfiguration.month_key || '-'}；腾讯开启时系统会按材料类型、识图强度、接口可用性和剩余额度自动选择已启用服务，每次真实请求保守计入一次，缓存命中不消耗额度。${imageGate}`;
  ...
  async function loadProfiles() {
    const [data, visionData, ocrFeatureData, ocrData] = await Promise.all([request('/model-profiles'), request('/vision-configuration'), request('/ocr-feature-configuration'), request('/tencent-ocr-configuration')]); modelProfiles = data.profiles; visionConfiguration = visionData.configuration || visionConfiguration; ocrFeatureConfiguration = ocrFeatureData.configuration || ocrFeatureConfiguration; ocrConfiguration = ocrData.configuration || ocrConfiguration; renderOcrConfiguration();
  ...
  const acquisitionPresetLabels = {smart:'智能取证', text:'扫描文字核验', visual:'签章/外观核验', dual:'双重核验', off:'不进行图片取证', custom:'自定义高级设置'};
  ...
  function acquisitionSummary(rule) {
    ...
    if (mode === 'off' || trigger === 'off') return {label:'未启用图片取证', state:'off', detail:'本规则只使用文字审查'};
    if (level === 'off') return {label:'图片取证待配置', state:'warning', detail:'已设置取证策略，但覆盖上限为关闭'};
    return {label:`${acquisitionPresetLabels[inferredAcquisitionPreset(rule)] || '智能取证'} · ${acquisitionLevelLabels[level] || '标准'}上限`, state:'active', detail:trigger === 'required' ? '每次均执行' : '关键证据不完整时执行'};
  }
  function acquisitionPreview(rule) {
    const mode = String(rule?.image_mode || 'auto'); const trigger = String(rule?.vision_trigger || 'off'); const level = String(rule?.vision_level || 'off');
    if (mode === 'off' || trigger === 'off' || level === 'off') return '当前不会启动 OCR 或多模态图片取证。';
    const budget = {low:'最多 2 页', standard:'OCR 最多 6 页；图片最多首批 4 页、可补 4 页', high:'OCR 最多 10 页；图片最多首批 6 页、可补 6 页'}[level] || '按当前覆盖上限执行';
    const path = {ocr_only:'先定位候选页，再提取扫描文字', vision_only:'先定位候选页，再核验图片外观', combined:'OCR 与图片外观均会执行', auto:'先定位候选页，再按材料与关键字段决定 OCR / 图片复核'}[mode] || '按规则决定取证路径';
    return `${path}；${budget}；证据提前完整时会停止。`;
  }
  function presetPayload(preset, level, rule = {}) {
    const presetMap = {
      smart:{image_mode:'auto', vision_trigger:'text_fallback'}, text:{image_mode:'ocr_only', vision_trigger:'text_fallback'},
  ...
  async function refreshRules() {
    ...
    const toolbar = isDraft && displayRules.length ? `<div class="rule-acquisition-toolbar"><span>图片取证可先按系统建议配置，再按需单条调整。</span><button class="restore-acquisition-recommendations" type="button">已启用规则恢复系统建议</button></div>` : '';
    $('rules').innerHTML = displayRules.length ? `${toolbar}<div class="rule-card-list">${displayRules.map((r) => {
      ...
      const preset = inferredAcquisitionPreset(r); const recommendation = r.acquisition_recommendation || {acquisition_preset:'off', vision_level:'off'};
      const visionControl = isDraft ? `<div class="rule-vision-controls"><div class="rule-vision-heading"><strong>图片取证</strong><small>仅影响本条规则；全局 OCR 与多模态开关保持不变。</small></div><label>取证策略<select class="rule-acquisition-preset" data-rule="${r.rule_id}"><option value="smart" ${preset === 'smart' ? 'selected' : ''}>智能取证（推荐）</option><option value="text" ${preset === 'text' ? 'selected' : ''}>扫描文字核验</option><option value="visual" ${preset === 'visual' ? 'selected' : ''}>签章/外观核验</option><option value="dual" ${preset === 'dual' ? 'selected' : ''}>文字与外观双重核验</option><option value="off" ${preset === 'off' ? 'selected' : ''}>不进行图片取证</option><option value="custom" ${preset === 'custom' ? 'selected' : ''}>自定义高级设置</option></select></label><label>覆盖上限<select class="rule-coverage-level" data-rule="${r.rule_id}"><option value="off" ${r.vision_level === 'off' ? 'selected' : ''}>关闭</option><option value="low" ${r.vision_level === 'low' ? 'selected' : ''}>快速</option><option value="standard" ${r.vision_level === 'standard' ? 'selected' : ''}>标准（推荐）</option><option value="high" ${r.vision_level === 'high' ? 'selected' : ''}>充分</option></select></label><div class="rule-acquisition-preview"><strong>执行预览</strong><span>${escapeHtml(acquisitionPreview(r))}</span></div><div class="rule-acquisition-actions"><button class="restore-rule-acquisition" data-rule="${r.rule_id}" type="button">恢复系统建议</button><small>建议：${escapeHtml(acquisitionPresetLabels[recommendation.acquisition_preset] || '智能取证')} · ${escapeHtml(acquisitionLevelLabels[recommendation.vision_level] || '标准')}上限</small></div><details class="rule-image-advanced"><summary>高级：通道与启动方式</summary><div class="rule-image-advanced-grid"><label>取证通道<select class="rule-image-mode" data-rule="${r.rule_id}"><option value="auto" ${r.image_mode === 'auto' ? 'selected' : ''}>自动协同</option><option value="ocr_only" ${r.image_mode === 'ocr_only' ? 'selected' : ''}>仅 OCR</option><option value="vision_only" ${r.image_mode === 'vision_only' ? 'selected' : ''}>仅多模态</option><option value="combined" ${r.image_mode === 'combined' ? 'selected' : ''}>OCR＋多模态</option><option value="off" ${r.image_mode === 'off' ? 'selected' : ''}>关闭</option></select></label><label>启动方式<select class="rule-vision-trigger" data-rule="${r.rule_id}"><option value="off" ${r.vision_trigger === 'off' ? 'selected' : ''}>不执行</option><option value="text_fallback" ${r.vision_trigger === 'text_fallback' ? 'selected' : ''}>关键证据不完整时执行</option><option value="required" ${r.vision_trigger === 'required' ? 'selected' : ''}>每次均执行</option></select></label></div></details><small>OCR：${ocrFeatureConfiguration.enabled ? '已开启（腾讯云优先或本地由全局设置决定）' : '已关闭'}；多模态：${visionConfiguration.enabled ? '已开启，可核验签章、版式等外观事实' : '已关闭'}。扫描文字核验不会向模型发送图片。</small></div>` : '';
    ...
    $('rules').querySelectorAll('.rule-acquisition-preset').forEach((input) => { input.onchange = async () => { try { const level = $('rules').querySelector(`.rule-coverage-level[data-rule="${input.dataset.rule}"]`)?.value || 'standard'; await saveAcquisition(input.dataset.rule, presetPayload(input.value, level, ruleById.get(input.dataset.rule))); await refreshRules(); } catch (error) { alert(error.message); await refreshRules(); } }; });
    $('rules').querySelectorAll('.rule-coverage-level').forEach((input) => { input.onchange = async () => { try { const preset = $('rules').querySelector(`.rule-acquisition-preset[data-rule="${input.dataset.rule}"]`)?.value || 'smart'; if (preset === 'custom') { const mode = $('rules').querySelector(`.rule-image-mode[data-rule="${input.dataset.rule}"]`)?.value || 'auto'; const trigger = $('rules').querySelector(`.rule-vision-trigger[data-rule="${input.dataset.rule}"]`)?.value || 'off'; await saveAcquisition(input.dataset.rule, {acquisition_preset:'custom', image_mode:mode, vision_trigger:trigger, vision_level:(mode === 'off' || trigger === 'off') ? 'off' : input.value}); } else { await saveAcquisition(input.dataset.rule, presetPayload(preset, input.value, ruleById.get(input.dataset.rule))); } await refreshRules(); } catch (error) { alert(error.message); await refreshRules(); } }; });
    $('rules').querySelectorAll('.rule-image-mode, .rule-vision-trigger').forEach((input) => { input.onchange = async () => { try { const level = $('rules').querySelector(`.rule-coverage-level[data-rule="${input.dataset.rule}"]`)?.value || 'off'; const mode = $('rules').querySelector(`.rule-image-mode[data-rule="${input.dataset.rule}"]`)?.value || 'auto'; const trigger = $('rules').querySelector(`.rule-vision-trigger[data-rule="${input.dataset.rule}"]`)?.value || 'off'; await saveAcquisition(input.dataset.rule, {acquisition_preset:'custom', image_mode:mode, vision_trigger:trigger, vision_level:(mode === 'off' || trigger === 'off') ? 'off' : level}); await refreshRules(); } catch (error) { alert(error.message); await refreshRules(); } }; });
  ...
  function resetGlobalRuleForm() { $('global-rule-id').value = ''; $('global-rule-form-title').textContent = '新增通用规则'; $('global-rule-category').value = 'substantive'; $('global-rule-title').value = ''; $('global-rule-check').value = ''; $('global-rule-source').value = ''; $('global-rule-acquisition').value = 'off'; $('global-rule-coverage').value = 'off'; $('global-rule-ocr').checked = false; $('global-rule-enabled').checked = true; }
  ...
  async function loadGlobalRules() {
    ...
    $('global-rules').querySelectorAll('.edit-global-rule').forEach((button) => button.onclick = () => { const rule = globalRules.find((item) => item.global_rule_id === button.dataset.rule); if (!rule) return; $('global-rule-id').value = rule.global_rule_id; $('global-rule-form-title').textContent = `编辑通用规则：${rule.title}`; $('global-rule-category').value = rule.category; $('global-rule-title').value = rule.title; $('global-rule-check').value = rule.check_rule; $('global-rule-source').value = rule.source_text || ''; $('global-rule-acquisition').value = inferredAcquisitionPreset(rule) === 'custom' ? 'smart' : inferredAcquisitionPreset(rule); $('global-rule-coverage').value = rule.vision_level || 'off'; $('global-rule-ocr').checked = rule.check_mode === 'ocr'; $('global-rule-enabled').checked = Boolean(rule.enabled); });
  ...
  $('save-global-rule').onclick = async () => { const password = window.prompt('请输入通用规则库操作口令'); if (password === null) return; try { const ruleId = $('global-rule-id').value; const preset = $('global-rule-acquisition').value; const level = $('global-rule-coverage').value; const payload = {category:$('global-rule-category').value, title:$('global-rule-title').value, check_rule:$('global-rule-check').value, source_text:$('global-rule-source').value, ocr_required:$('global-rule-ocr').checked, enabled:$('global-rule-enabled').checked, password, ...presetPayload(preset, level)}; const data = await request(ruleId ? `/global-rules/${ruleId}` : '/global-rules', {method:ruleId ? 'PATCH' : 'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload)}); resetGlobalRuleForm(); await loadGlobalRules(); if (!ruleId && activeProject) await refreshRules(); if (!ruleId && data.rule?.synced_draft_rule_sets) $('global-rule-form-title').textContent = `已同步至 ${data.rule.synced_draft_rule_sets} 个待确认项目`; } catch (error) { alert(error.message); } };
  ...
  $('ocr-feature-enabled').onchange = () => { $('ocr-route-controls').disabled = !$('ocr-feature-enabled').checked; };
  ...
  $('save-ocr-feature-configuration').onclick = async () => { try {
    const enabled = $('ocr-feature-enabled').checked;
    const tencentEnabled = $('ocr-route-tencent').checked;
    // “识别路线”与 OCR 总开关在同一次保存中落库，避免出现用户以为已切换、实际仍沿用旧路线的状态。
    await Promise.all([
      request('/ocr-feature-configuration', {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({enabled})}),
      request('/tencent-ocr-configuration', {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({tencent_enabled:tencentEnabled})}),
    ]);
    await loadProfiles();
  } catch (error) { alert(error.message); } };
  ...
  $('save-ocr-configuration').onclick = async () => { try { const services = {}; $('ocr-services').querySelectorAll('.ocr-service-enabled').forEach((input) => { const limit = $('ocr-services').querySelector(`.ocr-service-limit[data-service="${input.dataset.service}"]`); services[input.dataset.service] = {enabled:input.checked, monthly_limit:Number(limit?.value || 900)}; }); await request('/tencent-ocr-configuration', {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({tencent_enabled:$('ocr-route-tencent').checked, region:$('ocr-region').value, secret_id:$('ocr-secret-id').value, secret_key:$('ocr-secret-key').value, services})}); $('ocr-secret-id').value = ''; $('ocr-secret-key').value = ''; await loadProfiles(); } catch (error) { alert(error.message); } };
