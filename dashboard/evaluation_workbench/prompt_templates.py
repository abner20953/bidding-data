...
EVALUATION_PROMPT_VERSION = "vision-evidence-contract-v49"
...
PROMPT_TEMPLATES = {
    ...
    "extract_rules_source_ledger_contract": _template("评审规则提取 · 原文台账约束", "非评分规则必须引用稳定原文单元，并明确是否可由单份投标文件核验。", "本次原文按【原文单元 SU-*】标识。非评分规则必须在 JSON 中额外返回 source_unit_ids（仅可填写输入中出现的 SU-* ID）、verification_target（要从单份投标文件核验的具体事实）和 verifiability（single_bid|cross_bid|external_procedure）。只有无需其他投标人数据、评委现场判断、后续澄清或外部程序即可得出结论的规则，才可输出并填写 single_bid；其余事项直接忽略。source_unit_ids 可以包含同一底层事实的多个直接依据，但不得凭标题猜测来源。"),
    "extract_rules_guidance": _template("评审规则提取 · 通用业务指令", "独立于任务模板的可维护规则提取原则；用于持续完善业务判断。", "优先完整、准确地提取可由投标文件审查的要求，但最终规则必须少而精。只保留可由当前已提交的电子投标文件单独证实或证伪、并会独立改变评审结论或分值的事实；不得把后续合同签订、履约、履约保证金、采购数量调整，或评审过程中才发生的澄清、补正、谈判、异常低价说明、算术更正、投诉等写成投标文件审查规则。串通、行贿、弄虚作假、失信或停业等外部事实也不能仅凭单份投标文件判定；除非招标文件只要求核验投标人是否作出相应书面声明，否则应留给查重或外部核验流程。一个底层审查事实只保留一条规则：材料要求、证明方式和缺失后果应合并写入同一 check_rule，不得分别生成“应提供凭证”和“未提供则无效”；同一响应字段中的期限、地点、标准、金额或税费口径应合并为一条完整规则；章节总标题、资料目录、格式模板说明、已被具体子规则覆盖的概括性要求不得单独成规则。对业绩、报价、人员、资质、技术方案等评分条款，必须保留有效期、证明材料、计分对象、公式和上限。评分表存在层级时，必须先识别叶子评分项及其父项总分：如“7 个服务模块、每项 3 分、总计 21 分”，应形成一条可执行评分规则，check_rule 中逐项写明 7 个模块、每项分值、评价维度（完整性、合理性、可靠性、前瞻性、适用性、可维护性等）及“缺项/缺陷”的扣分方式；不得只留下“系统功能先进性/技术方案”这类空泛标题，也不得把 7 个模块误合并为一个无计分依据的概述。电子投标文件无法可靠核验的纸质正副本标记及一致性、装订、密封、份数、现场递交等事项不成规则；前附表、星号或交叉引用未给出可核验的具体条件时，也不生成笼统规则。若最终判断必须查看证照、许可证、执照、签字盖章、手写内容、扫描凭证、保函、票据或其他图片外观，ocr_required 必须为 true；仅目录出现或文本提取到证件名称不代表已经核验图像。若文字、表格已能给出该规则的决定性结论，不能仅因附带材料可能是扫描件就把整条规则标为 OCR；只有图像外观本身决定结论时才标 true，必要时将可独立判断的文字条件和图像核验条件拆成两条规则。对按“每缺一项不得分、每处缺陷扣 0.5 分、扣完为止”等规则，应原样保留触发条件和封顶逻辑。遇到语义相近条款时，先保留原始依据，由来源单元与检查意图进行确定性收口；不得根据固定业务词表删除条款。输出前逐项通过三道门：第一，结论能否只凭本次投标文件得出，不能则删除；第二，它是否是一个未被更具体规则承接的独立审查事实，不能则合并或删除；第三，OCR 是否确为决定性证据，不能则设为 false。"),
    ...
    "extract_rules_obligation_compile_user": _template("评审规则提取 · 义务汇总编译", "全局汇总已锚定的非评分候选；只返回候选分组，不生成新规则文字。", "以下候选均已通过原文来源、分包和单份投标文件核验门控。请把它们划分为最终审查义务组，只返回合法 JSON：\n{\"groups\":[{\"candidate_ids\":[\"O-1\",\"O-2\"],\"action\":\"merge\"},{\"candidate_ids\":[\"O-3\"],\"action\":\"keep_separate\"}]}\n\n每个 O-* 必须且只能出现一次。以“少而精、一个底层审查事实一张规则卡”为目标：同一投标文件事实被不同章节重复表述，或同一材料/字段/响应要求及其后果被拆成多条时必须 merge；不得因标题、类别或来源页不同而保留重复卡。可把同一总体响应要求与其纯分组/分段子要求合并，系统会保留全部子检查项。不同材料、不同字段、不同适用条件、不同独立证明要求或可分别成立的不响应事实必须 keep_separate。不得仅依据标题相似就合并；不得新增、删除、改写候选，不得改变评分、原文、OCR 或来源。\n\n候选：\n{{candidates}}", "candidates"),
