...
PROMPT_TEMPLATES = {
    ...
    "evaluate_all_highlights_user": _template("综合评审 · 重要结论提炼", "综合评审完成后，从已有结论中提炼最值得优先复核的事项。", "请仅根据下列已有评审候选提炼极其重要的结论，不重新评审、不补充新事实，只返回 JSON：\n{\"summaries\":[{\"document_id\":\"投标文件ID\",\"bidder_name\":\"投标人\",\"overall_level\":\"critical|high|attention|none\",\"headline\":\"一句话总览\",\"highlights\":[{\"rule_id\":\"规则ID\",\"level\":\"critical|high|attention\",\"keyword\":\"需要加粗的短关键词\",\"conclusion\":\"简洁结论\",\"basis\":\"最关键证据与理由\"}]}]}\n\n严格口径：1. critical 仅用于资格、符合性、实质性或明确废标规则，且已有结果同时满足 status=not_satisfied、risk_level=high、confidence=high、evidence_quality=sufficient，并且规则原文明确存在投标无效、否决、不通过等后果；任何条件不满足都不得使用“明确废标”“确定否决”等措辞。2. high 用于证据较强但仍需人工最终确认的重大不响应或严重失分；attention 用于证据有限、部分满足、未找到材料或重大低分线索。3. OCR 待识别、普通人工判断、低风险和证据缺失事项不得升级成 critical。4. keyword 最多16字，conclusion 最多80字，basis 最多140字；不得复述整条检查规则，不得使用 Markdown，不得编造证据。5. 每个投标人最多5条，按重要程度排序；没有足够重要结论时 highlights 返回空数组、overall_level=none。\n\n已有评审候选：\n{{candidates}}", "candidates"),
