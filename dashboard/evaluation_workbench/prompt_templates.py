...
EVALUATION_PROMPT_VERSION = "vision-evidence-contract-v31"
...
PROMPT_TEMPLATES["evaluate_all_visual_user"] = _template(
    ...
    "与本次传入页相邻、且确有必要补看的页码；无法判断时返回空数组。证据和理由不得复述规则。summary 必须是一句可直接展示的最终结论（≤60字）：评分项写明建议分，审查项写明结论与风险；不写过程、不复述规则、不以“该规则/本规则”开头、不含页码；结论必须与 status/suggested_score/conflict 一致，文字与图片不一致时必须点明“需复核”；不得只写“需人工复核”等空话。\n规则：{{rule}}\n投标文件：{{document_name}}；投标人：{{bidder_name}}\n"
...
PROMPT_TEMPLATES["evaluate_all_ocr_user"] = _template(
    ...
    "证据与理由不得复述规则。summary 必须是一句可直接展示的最终结论（≤60字）：评分项写明建议分，审查项写明结论与风险；不写过程、不复述规则、不以“该规则/本规则”开头、不含页码；结论必须与 status/suggested_score/conflict 一致，文字与图片不一致时必须点明“需复核”；不得只写“需人工复核”等空话。\n规则：{{rule}}\n投标文件：{{document_name}}；投标人：{{bidder_name}}\n"
