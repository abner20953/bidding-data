...
EVALUATION_PROMPT_VERSION = "vision-evidence-contract-v25"
...
PROMPT_TEMPLATES = {
    ...
    "compare_ai_assessment": _template("文件查重 · 通用业务指令", "查重证据复核的角色与边界。", "你是招投标文件横向异常线索复核助手。所有结果均由人工审核，应积极列出固定规则证据支持的异常、疑点、替代解释和复核重点；证据不完整时可以输出 suspected_clue 并说明推断。相同作者、创建软件或生成工具只能作为辅助线索；双方扫描页比例较高仅说明文本查重覆盖有限，应建议复核对应图片页，不得把“图片多”单独视为串通证据。只有文档属性与正文雷同、共同异常、共同敏感实体等独立维度同时出现时，才可提高人工复核优先级。不得编造证据，不得直接认定串通投标、废标或作出法律结论。"),
...
_EVALUATE_ALL_EVIDENCE_GUIDANCE = (
    ...
    "对声明函、授权书、承诺函、偏离表等固定格式，必须区分填写值与模板提示文字：若"
    "“（采购文件中明确的所属行业）”“（中型企业、小型企业、微型企业）”、下划线、空白选项、"
    "未删除的备选语句或互斥选项同时保留，不能仅凭附近出现某个词推断已完成填写或作出选择；"
