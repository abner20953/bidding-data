...
PROMPT_TEMPLATES = {
    ...
    "compare_ai_assessment_user": _template("文件查重 AI 复核 · 任务", "压缩证据包及 JSON 输出要求。", "请按固定规则复核以下压缩证据包，只返回 JSON：\n{\"assessments\":[{\"signal_id\":\"ID\",\"decision\":\"confirmed_clue|suspected_clue|excluded|unassessable\",\"risk_level\":\"low|medium|high\",\"confidence\":\"high|medium|low\",\"source_class\":\"bidder_specific|third_party_common|public_template|placeholder_or_extraction|unknown\",\"materiality\":\"substantive|formatting|unknown\",\"reason\":\"简洁理由\",\"suggested_check\":\"建议核验事项\"}]}\n\n判定含义：confirmed_clue 仅表示当前信号本身有较充分证据；suspected_clue 表示仍有合理替代解释；excluded 表示更可能为招标原文、固定表单、公开产品资料、行业通用参数或 PDF 提取伪差异；unassessable 表示证据不足。source_class 必须说明线索来源属性：投标人独有、共同第三方资料、公共模板、占位/提取伪影或无法判断；materiality 区分实质内容与纯格式。公共模板、共同第三方资料、占位值或提取伪影即使暂列疑似，也不得作为提高文件对优先级的依据。basis 中的数量是底层候选数，不可直接当作独立异常数量。必须只依据当前 signal_id 的证据独立判断，禁止引用“其他维度叠加”“同组另有线索”来提高风险。技术参数相似本身通常不足以判高风险；只有共同的非公共独有内容、可验证罕见错误或实质异常改动才可升高。assessment_scope=project_entity_cluster 时表示同一实体在三家以上投标文件出现：应一次判断其业务归属，不得因出现次数本身抬高风险。必须按输入顺序覆盖每个 signal_id，且每个 ID 恰好返回一次，不得新增 ID。reason 最多120字，点明真正有判别力的事实或公共来源解释；suggested_check 最多80字；避免复述整段证据。不得输出串标成立、废标或扣分结论。\n证据包：{{packets}}", "packets"),
