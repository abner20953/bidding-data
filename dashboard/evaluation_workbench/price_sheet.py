...
def _compile_price_formula(rule: dict) -> dict:
    ...
    trim_mode = "none"
    trim_rounding = "floor"
    # 按家数比例（如 20%）修剪的表述有两种常见顺序：“去掉最高/最低报价的 20%”
    # 和“按其 20%（四舍五入取整）去掉最高和最低”。两者都表示按家数比例修剪，
    # 且通常伴随“≥N 家时”的适用条件；必须与“去掉一个最高和一个最低”的固定
    # 数量修剪严格区分，避免把比例修剪误判为固定数量修剪。
    if re.search(
        r"(?:去掉|剔除).{0,28}(?:最高|最低).{0,28}(?:20%|百分之二十)"
        r"|(?:20%|百分之二十)[^。；;]{0,36}?(?:去掉|剔除)[^。；;]{0,24}(?:最高|最低)",
        text,
    ):
